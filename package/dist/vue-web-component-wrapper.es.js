var k = Object.defineProperty;
var F = (e, r, t) => r in e ? k(e, r, { enumerable: !0, configurable: !0, writable: !0, value: t }) : e[r] = t;
var m = (e, r, t) => F(e, typeof r != "symbol" ? r + "" : r, t);
import { defineComponent as I, nextTick as W, render as D, createVNode as X, h as $ } from "vue";
/**
* @vue/shared v3.5.13
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
process.env.NODE_ENV !== "production" && Object.freeze({});
process.env.NODE_ENV !== "production" && Object.freeze([]);
const Z = Object.assign, C = Array.isArray, J = (e) => typeof e == "string", B = (e) => {
  const r = /* @__PURE__ */ Object.create(null);
  return (t) => r[t] || (r[t] = e(t));
}, Q = /-(\w)/g, L = B(
  (e) => e.replace(Q, (r, t) => t ? t.toUpperCase() : "")
), Y = /\B([A-Z])/g, N = B(
  (e) => e.replace(Y, "-$1").toLowerCase()
), R = (e) => {
  const r = J(e) ? Number(e) : NaN;
  return isNaN(r) ? e : r;
};
/*!#__NO_SIDE_EFFECTS__*/
// @__NO_SIDE_EFFECTS__
function ee(e, r, t) {
  const s = I(e);
  class o extends P {
    constructor(i) {
      super(s, i, r, t);
    }
  }
  return m(o, "def", s), o;
}
const te = typeof HTMLElement < "u" ? HTMLElement : class {
};
class P extends te {
  constructor(t, s = {}, o = { shadowRoot: !0 }, n) {
    super();
    /**
     * @internal
     */
    m(this, "_instance", null);
    m(this, "_connected", !1);
    m(this, "_resolved", !1);
    m(this, "_numberProps", null);
    m(this, "_styles");
    m(this, "_slots", {});
    m(this, "_ob", null);
    this._def = t, this._props = s, this._config = o, this._root && n ? n(this._createVNode(), this._root) : (this._config.shadowRoot !== !1 && this.attachShadow({ mode: "open" }), this._def.__asyncLoader || this._resolveProps(this._def));
  }
  get _root() {
    return this._config.shadowRoot ? this.shadowRoot : this;
  }
  connectedCallback() {
    this._connected = !0, this._instance || (this._resolved ? this._update() : this._resolveDef());
  }
  disconnectedCallback() {
    this._connected = !1, W(() => {
      this._connected || (this._ob && (this._ob.disconnect(), this._ob = null), D(null, this._root), this._instance = null);
    });
  }
  /**
   * resolve inner component definition (handle possible async component)
   */
  _resolveDef() {
    this._resolved = !0;
    for (let o = 0; o < this.attributes.length; o++)
      this._setAttr(this.attributes[o].name);
    this._ob = new MutationObserver((o) => {
      for (const n of o)
        this._setAttr(n.attributeName);
    }), this._ob.observe(this, { attributes: !0 });
    const t = (o, n = !1) => {
      var y;
      const { props: i } = o, _ = this._collectNestedStyles(o);
      let E;
      if (i && !C(i))
        for (const h in i) {
          const c = i[h];
          (c === Number || c && c.type === Number) && (h in this._props && (this._props[h] = R(this._props[h])), (E || (E = /* @__PURE__ */ Object.create(null)))[L(h)] = !0);
        }
      if (this._numberProps = E, n && this._resolveProps(o), !this._config.shadowRoot) {
        this._slots = {};
        const h = (c) => Array.from(c).map((l) => {
          var p;
          if (l.nodeType === Node.ELEMENT_NODE) {
            const u = l, b = Object.fromEntries(
              Array.from(u.attributes).map((O) => [O.name, O.value])
            );
            return $(
              u.tagName.toLowerCase(),
              b,
              h(u.childNodes)
            );
          } else if (l.nodeType === Node.TEXT_NODE)
            return ((p = l.textContent) == null ? void 0 : p.trim()) || null;
          return null;
        }).filter((l) => l != null);
        for (const c of Array.from(this.childNodes)) {
          const l = c.nodeType === Node.ELEMENT_NODE && c.getAttribute("slot") || "default";
          if (this._slots[l] || (this._slots[l] = []), c.nodeType === Node.ELEMENT_NODE) {
            const p = c, u = Object.fromEntries(
              Array.from(p.attributes).map((b) => [b.name, b.value])
            );
            this._slots[l].push(
              $(
                p.tagName.toLowerCase(),
                u,
                h(p.childNodes)
              )
            );
          } else if (c.nodeType === Node.TEXT_NODE) {
            const p = (y = c.textContent) == null ? void 0 : y.trim();
            p && this._slots[l].push(p);
          }
        }
        this.replaceChildren();
      }
      this._applyStyles(_), this._update();
    }, s = this._def.__asyncLoader;
    s ? s().then((o) => t(o, !0)) : t(this._def);
  }
  _resolveProps(t) {
    const { props: s } = t, o = C(s) ? s : Object.keys(s || {});
    for (const n of Object.keys(this))
      n[0] !== "_" && o.includes(n) && this._setProp(n, this[n], !0, !1);
    for (const n of o.map(L))
      Object.defineProperty(this, n, {
        get() {
          return this._getProp(n);
        },
        set(i) {
          this._setProp(n, i);
        }
      });
  }
  _setAttr(t) {
    let s = this.hasAttribute(t) ? this.getAttribute(t) : void 0;
    const o = L(t);
    this._numberProps && this._numberProps[o] && (s = R(s)), this._setProp(o, s, !1);
  }
  /**
   * @internal
   */
  _getProp(t) {
    return this._props[t];
  }
  /**
   * @internal
   */
  _setProp(t, s, o = !0, n = !0) {
    s !== this._props[t] && (this._props[t] = s, n && this._instance && this._update(), o && (s === !0 ? this.setAttribute(N(t), "") : typeof s == "string" || typeof s == "number" ? this.setAttribute(N(t), s + "") : s || this.removeAttribute(N(t))));
  }
  _update() {
    D(this._createVNode(), this._root);
  }
  _createVNode() {
    const t = X(this._def, Z({}, this._props), this._slots);
    return this._instance || (t.ce = (s) => {
      this._instance = s, s.isCE = !0;
      const o = (i, _) => {
        this.dispatchEvent(
          new CustomEvent(i, {
            detail: _
          })
        );
      };
      s.emit = (i, ..._) => {
        o(i, _), N(i) !== i && o(N(i), _);
      };
      let n = this;
      for (; n = n && (n.parentNode || n.host); )
        if (n instanceof P) {
          s.parent = n._instance, s.provides = n._instance.provides;
          break;
        }
    }), t;
  }
  _applyStyles(t) {
    console.log("applyStyles", t), t && t.forEach((s) => {
      const o = document.createElement("style");
      o.textContent = s, this._config.nonce && o.setAttribute("nonce", this._config.nonce), this._root.prepend(o);
    });
  }
  _collectNestedStyles(t) {
    let s = t.styles ?? [];
    return t.components && Object.values(t.components).forEach((o) => {
      s = s.concat(this._collectNestedStyles(o));
    }), s;
  }
}
const H = (e) => {
  for (; (e == null ? void 0 : e.nodeType) !== 1; ) {
    if (!e.parentElement)
      throw new Error(
        "No parent element found, the rootComponent must be wrapped in a HTML element (e.g. <template><div> app content </div></template>)"
      );
    e = e.parentElement;
  }
  return e;
};
function se(e) {
  return "on" + e.charAt(0).toUpperCase() + e.slice(1);
}
function oe(e) {
  return typeof e == "string" ? e.replace(/:root/g, ":host") : Array.isArray(e) ? e.map((r) => r.replace(/:root/g, ":host")) : e;
}
const ne = ({
  rootComponent: e,
  plugins: r,
  cssFrameworkStyles: t,
  VueDefineCustomElement: s,
  h: o,
  createApp: n,
  getCurrentInstance: i,
  elementName: _,
  disableRemoveStylesOnUnmount: E,
  disableShadowDOM: y,
  replaceRootWithHostInCssFramework: h,
  asyncInitialization: c,
  loaderAttribute: l,
  hideSlotContentUntilMounted: p,
  nonce: u
}) => {
  console.log("defineCustomElement called");
  const b = y ? ee : s, O = h ? oe(t) : t, M = b({
    styles: [O],
    nonce: u,
    props: {
      ...e.props,
      modelValue: { type: [String, Number, Boolean, Array, Object] }
      // v-model support
    },
    emits: e == null ? void 0 : e.emits,
    setup(K, { slots: U }) {
      var j;
      console.log("HELLLOOOO");
      const T = [...(e == null ? void 0 : e.emits) || [], "update:modelValue"], f = n();
      if (f.component("app-root", e), e.provide) {
        const a = typeof e.provide == "function" ? e.provide() : e.provide;
        Object.keys(a).forEach((d) => {
          f.provide(d, a[d]);
        });
      }
      f.mixin({
        mounted() {
          var w, A, V, x;
          if (((A = (w = this.$) == null ? void 0 : w.type) == null ? void 0 : A.name) === "vue-custom-element-root-component")
            return;
          const a = (v) => {
            if (v != null && v.length) {
              this.__style = document.createElement("style"), this.__style.innerText = v.join().replace(/\n/g, ""), u && this.__style.setAttribute("nonce", u);
              const S = document.querySelector("style")[-1];
              H(S ?? this.$el).append(this.__style);
            }
          };
          if (this.$el.appendChild(document.createElement("div").setAttribute("id", "TESTING")), a((V = this.$) == null ? void 0 : V.type.styles), this.$options.components)
            for (const v of Object.values(this.$options.components))
              a(v.styles);
          const d = ((x = this.$el.getRootNode()) == null ? void 0 : x.host) || H(this.$el);
          d && (p && d.querySelectorAll("[hidden]").forEach((G) => {
            G.removeAttribute("hidden");
          }), d.querySelectorAll(`[${l}]`).forEach((S) => {
            S.remove();
          }));
        },
        unmounted() {
          var a;
          E || (a = this.__style) == null || a.remove();
        }
      }), f.use(r);
      const g = i();
      if (Object.assign(g.appContext, f._context), Object.assign(g.provides, f._context.provides), process.env.NODE_ENV === "development" && window.__VUE_DEVTOOLS_GLOBAL_HOOK__) {
        const a = document.querySelector(_);
        f._container = a, f._instance = g;
        const d = {
          Comment: Symbol("v-cmt"),
          Fragment: Symbol("v-fgt"),
          Static: Symbol("v-stc"),
          Text: Symbol("v-txt")
        };
        window.__VUE_DEVTOOLS_GLOBAL_HOOK__.emit("app:init", f, f.version, d), window.__VUE_DEVTOOLS_GLOBAL_HOOK__.Vue = f;
      }
      const q = T == null ? void 0 : T.reduce((a, d) => {
        const w = se(d);
        return a[w] = (A) => {
          g.emit(d, A);
        }, a;
      }, {}), z = (j = e == null ? void 0 : e.namedSlots) == null ? void 0 : j.reduce((a, d) => (a[d] = () => o("slot", {
        name: d
      }), a), {});
      return () => o(
        e,
        {
          ...K,
          ...q
        },
        {
          default: () => o("slot"),
          ...z,
          ...U
        }
      );
    }
  }, { shadowRoot: !y, nonce: u });
  return c().then(() => M);
}, ce = ({
  elementName: e,
  rootComponent: r,
  plugins: t,
  cssFrameworkStyles: s,
  VueDefineCustomElement: o,
  h: n,
  createApp: i,
  getCurrentInstance: _,
  disableRemoveStylesOnUnmount: E = !1,
  disableShadowDOM: y = !1,
  replaceRootWithHostInCssFramework: h = !1,
  asyncInitialization: c = () => Promise.resolve(),
  loaderAttribute: l = "data-web-component-loader",
  hideSlotContentUntilMounted: p = !1,
  nonce: u
  // Used for Content Security Policy (CSP) compliance - will be applied to inline styles
}) => {
  if (!r) {
    console.warn("No root component provided. Please provide a root component to create a web component.");
    return;
  }
  if (!e) {
    console.warn("No element name provided. Please provide an element name to create a web component.");
    return;
  }
  if (!o) {
    console.warn(
      "No VueDefineCustomElement provided. Please provide a VueDefineCustomElement to create a web component."
    );
    return;
  }
  if (!n) {
    console.warn("No h provided. Please provide an h to create a web component.");
    return;
  }
  if (!i) {
    console.warn("No createApp provided. Please provide a createApp to create a web component.");
    return;
  }
  if (!_) {
    console.warn("No getCurrentInstance provided. Please provide a getCurrentInstance to create a web component.");
    return;
  }
  ne({
    rootComponent: r,
    plugins: t,
    cssFrameworkStyles: s,
    VueDefineCustomElement: o,
    h: n,
    createApp: i,
    getCurrentInstance: _,
    elementName: e,
    disableRemoveStylesOnUnmount: E,
    disableShadowDOM: y,
    replaceRootWithHostInCssFramework: h,
    asyncInitialization: c,
    loaderAttribute: l,
    hideSlotContentUntilMounted: p,
    nonce: u
  }).then((b) => {
    customElements.define(
      e,
      b
    );
  });
};
export {
  ce as createWebComponent,
  ce as default,
  ne as defineCustomElement,
  ee as defineCustomElementSFC
};
